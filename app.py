...
def run_diagnostics(data: pd.DataFrame):
    ...
    for _, row in data.iterrows():
        ...
        if abs(row['exhaust_temp'] - avg_exhaust) > 15:
            alerts.append(f"Cylinder {cyl_id}: Exhaust temp ({row['exhaust_temp']}°C) deviates from average ({avg_exhaust:.1f}°C).")
            
        # Rule 2: Pressure Imbalance (Max ±3 bar)
        if abs(row['p_max'] - avg_pmax) > 3:
            alerts.append(f"Cylinder {cyl_id}: Pmax ({row['p_max']} bar) deviates from average ({avg_pmax:.1f} bar).")
...
try:
    ...
    if uploaded_file:
        with st.spinner("Extracting and Validating Thermodynamic Data..."):
            time.sleep(1) # Brief pause to allow UI to render smoothly before processing
    ...
    st.error("CRITICAL SYSTEM FAILURE: An unexpected error occurred.")
    st.write(e)
